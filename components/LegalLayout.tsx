...
function LinkifiedText({ text }: { text: string }) {
  ...
  const tokens = [
    { match: email, node: (
      <a key="email" href={`mailto:${email}`} className="font-medium text-gold-ink hover:underline">
        {email}
      </a>
    ) },
    { match: phone, node: (
      <a key="phone" href={`tel:${phoneHref}`} className="font-medium text-gold-ink hover:underline">
        {phone}
      </a>
    ) },
  ];
    ...
    <>
      ...
      <section className="bg-navy-deep pt-28 text-white">
        <Container className="py-14">
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-gold-ink-light">
            Legal
          </p>
                ...
                {doc.sections.map((s) => (
                  <li key={s.heading}>
                    <a
                      ...
                      className="text-navy/60 hover:text-gold-ink"
                    >
                      {s.heading}
                    </a>
                  </li>
          ...
          <article className="max-w-3xl">
            ...
            <div className="mt-14 rounded-3xl border border-outline bg-white p-6 shadow-glass">
              ...
              <dl className="mt-4 space-y-2 text-sm text-navy/75">
                ...
                <div>
                  <dt className="inline font-semibold text-navy/60">Udyam: </dt>
                  <dd className="inline">{company.udyam}</dd>
                </div>
                <div>
